	...
	const struct elf32_phdr *phdr)
{
	...
	if (!fw_mem_base) {
		IPAERR("Failed to map %pK for the size of %u\n",
...
int ipa3_load_fws(const struct firmware *firmware, phys_addr_t gsi_mem_base)
{
	...
	if (phdr->p_vaddr != (gsi_mem_base + gsi_iram_ofst)) {
		IPAERR(
			"Invalid GSI FW img load addr vaddr=%pK gsi_mem_base=%pK gsi_iram_ofst=0x%lx\n"
	...
	if (phdr->p_vaddr != (ipa_reg_mem_base + ipa_reg_ofst)) {
		IPAERR(
			"Invalid IPA DPS img load addr vaddr=%pK ipa_reg_mem_base=%pK ipa_reg_ofst=%u\n"
	...
	if (phdr->p_vaddr != (ipa_reg_mem_base + ipa_reg_ofst)) {
		IPAERR(
			"Invalid IPA HPS img load addr vaddr=%pK ipa_reg_mem_base=%pK ipa_reg_ofst=%u\n"
			, phdr->p_vaddr, &ipa_reg_mem_base, ipa_reg_ofst);
		return -EINVAL;
	}
